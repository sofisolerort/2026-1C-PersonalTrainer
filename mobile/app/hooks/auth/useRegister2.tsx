...
export const useRegister2 = (registerData: RegisterData) => {
  ...
  const validate = (): boolean => {
    ...
    if (!form.nombre_completo.trim()) newErrors.nombre_completo = 'Nombre completo requerido';
    ...
    if (!form.lugar_entrenamiento) newErrors.lugar_entrenamiento = 'Selecciona un lugar';
  ...
  const registerComplete = async (onSuccess: () => void) => {
    ...
    try {
      ...
      if (signInError) throw new Error(`Error al iniciar sesión: ${signInError.message}`);
