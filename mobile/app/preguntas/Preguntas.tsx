...
const opcionesObjetivo = ['Bajar de peso', 'Ganar masa muscular', 'Mejorar fuerza', 'Mejorar resistencia', 'Mejorar salud general', 'Otro'];
const opcionesDias = ['1-2', '3-4', '5-6'];
const opcionesTiempo = ['Menos de 30 min', '30-45 min', '45-60 min', 'Más de 60 min'];
const opcionesLugar = ['Gimnasio', 'Casa', 'Aire libre'];
const opcionesCompromiso = ['Muy comprometido', 'Comprometido', 'Poco comprometido'];
...
export default function Onboarding() {
        ...
        .insert([
          { 
            ...
            datos_onboarding: {
              ...
              impedimentoFisicoDetalle: data.tieneImpedimento ? data.impedimentoFisicoDetalle : 'Ninguno',
  ...
  const SelectorBooleano = ({ label, name }: { label: string, name: keyof OnboardingFormData }) => (
    <View style={styles.selectorContainer}>
        ...
        render={({ field: { onChange, value } }) => (
          <View style={styles.opcionesGrid}>
            {opcionesSiNo.map((opcion) => (
              <TouchableOpacity
                key={opcion.label}
                style={[styles.opcionBoton, value === opcion.value && styles.opcionSeleccionada]}
                onPress={() => onChange(opcion.value)}
              >
                <Text style={[styles.opcionTexto, value === opcion.value && styles.opcionTextoSeleccionado]}>
                  {opcion.label}
                </Text>
              </TouchableOpacity>
            ))}
      ...
      />
      {errors[name] && <Text style={styles.errorText}>Selecciona una opción</Text>}
    </View>
  );

  // Componente Reutilizable Interno para opciones Múltiples de Texto (Strings)
  const SelectorTexto = ({ label, name, opciones }: { label: string, name: keyof OnboardingFormData, opciones: string[] }) => (
    <View style={styles.selectorContainer}>
      <Text style={authStyles.label}>{label}</Text>
      <Controller
        control={control}
        ...
        render={({ field: { onChange, value } }) => (
          <View style={styles.opcionesGrid}>
            {opciones.map((opcion) => (
              <TouchableOpacity
                key={opcion}
                style={[styles.opcionBoton, value === opcion && styles.opcionSeleccionada]}
                onPress={() => onChange(opcion)}
              >
                <Text style={[styles.opcionTexto, value === opcion && styles.opcionTextoSeleccionado]}>
                  {opcion}
                </Text>
              </TouchableOpacity>
            ))}
  ...
  return (
    <ScrollView contentContainerStyle={authStyles.container} showsVerticalScrollIndicator={false}>
      <View style={authStyles.card}>
        ...
        {step === 1 && (
          <View>
            <Controller
              ...
              render={({ field: { onChange, value } }) => (
                <CustomInput label="Nombre Completo" placeholder="Juan Pérez" value={value} onChangeText={onChange} />
              )}
            />
            {errors.nombreCompleto && <Text style={styles.errorText}>{errors.nombreCompleto.message}</Text>}
            ...
            <Controller
              ...
              render={({ field: { onChange, value } }) => (
                <CustomInput label="Edad" placeholder="25" keyboardType="numeric" value={value} onChangeText={onChange} />
              )}
            />
            {errors.edad && <Text style={styles.errorText}>{errors.edad.message}</Text>}

            <SelectorBooleano label="¿Entrenaste anteriormente?" name="entrenoAntes" />
            <SelectorBooleano label="¿Entrenas actualmente?" name="entrenaActualmente" />
        ...
        {step === 2 && (
          <View>
            <SelectorBooleano label="¿Tenés algún impedimento físico?" name="tieneImpedimento" />
            ...
            {tieneImpedimento === true && (
              <View>
                <Controller
                  ...
                  rules={{ required: 'Por favor, detalla tu impedimento para tu entrenador.' }}
                  render={({ field: { onChange, value } }) => (
                    <CustomInput 
                ...
                />
                {errors.impedimentoFisicoDetalle && <Text style={styles.errorText}>{errors.impedimentoFisicoDetalle.message}</Text>}
              </View>
            )}

            <SelectorTexto label="¿Cuál es tu objetivo principal?" name="objetivoPrincipal" opciones={opcionesObjetivo} />
            ...
            <View style={styles.rowBotones}>
              ...
              <CustomButton title="Siguiente" style={{ flex: 1 }} onPress={() => setStep(3)} />
            </View>
          </View>
        )}
        ...
        {step === 3 && (
          <View>
            <SelectorTexto label="¿Cuántos días por semana podés entrenar?" name="diasSemana" opciones={opcionesDias} />
            <SelectorTexto label="¿Cuánto tiempo podés dedicar por sesión?" name="tiempoSesion" opciones={opcionesTiempo} />
            <SelectorTexto label="¿Dónde preferís entrenar?" name="lugarEntrenamiento" opciones={opcionesLugar} />
            <SelectorTexto label="¿Qué tan comprometido te sentís para cumplir el plan?" name="compromiso" opciones={opcionesCompromiso} />

            <View style={styles.rowBotones}>
              <TouchableOpacity style={styles.botonAtras} onPress={() => setStep(2)}>
                <Text style={styles.textoAtras}>Atrás</Text>
              </TouchableOpacity>
              <CustomButton title="Finalizar" style={{ flex: 1 }} onPress={handleSubmit(onSubmit)} />
            </View>
          </View>
        )}
      </View>
